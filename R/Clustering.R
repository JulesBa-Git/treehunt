#' @keywords internal
#' @noRd
.extract_cocktail_list <- function(cocktails) {
  if (!is.data.frame(cocktails)) {
    if (!is.list(cocktails)) {
      stop("'cocktails' must be a list or data frame.", call. = FALSE)
    }
    return(cocktails)
  }

  if ("vec_cocktails" %in% names(cocktails)) {
    return(cocktails[["vec_cocktails"]])
  }

  list_columns <- names(cocktails)[vapply(cocktails, is.list, logical(1))]
  if (length(list_columns) != 1L) {
    stop(
      "A cocktail data frame must contain 'vec_cocktails' or exactly one list-column.",
      call. = FALSE
    )
  }
  cocktails[[list_columns]]
}
...
#'
#' @param cocktails A list of 1-based integer vectors or a data frame containing
#'   them. For a data frame, `vec_cocktails` is the preferred list-column name;
#'   otherwise exactly one list-column must be present.
