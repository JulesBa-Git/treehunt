...
GeneticAlgorithm<TargetType>::GeneticAlgorithm(
  ...
  if (seed_population.isNotNull()) {
    ...
    if (seed.size() > params.population_size)
      Rcpp::stop(
          "Seed population size (%i) exceeds the total population size (%i).",
    ...
    for (int i = 0; i < seed.size(); ++i) {
      ...
      if (tmp_sol.is_valid(data.get_tree()))
        ...
        Rcpp::Rcout << "Warning: Seed individual at index " << i
                    << " is invalid and was skipped.\n";
    }
  }
}

template <typename TargetType> void GeneticAlgorithm<TargetType>::initialize() {
  std::uniform_int_distribution<size_t> node(
      0, data_.get_tree().get_depth().size() - 1);
  ...
  for (size_t i = population_.size(); i < params_.population_size; ++i) {
    ...
    population_.emplace_back(
        Solution::create_random_valid(data_.get_tree(), rng_, solution_length));
  }
...
template <typename TargetType> void GeneticAlgorithm<TargetType>::evaluate() {

  for (Solution &solution : population_) {
    auto search = score_hash_map_.find(solution);
    if (search != score_hash_map_.end()) {
      ++cache_hits_;
      solution.set_score(search->second);
    } else {
      solution.set_score(compute_score(solution));
      score_hash_map_.insert({solution, solution.get_score()});
    }
  }
}

template <typename TargetType> void GeneticAlgorithm<TargetType>::penalize() {
  auto [M, index] = dissimilarity_input_computation();

  std::vector<std::vector<double>> similarity(
      population_.size(), std::vector<double>(population_.size(), -1.0));
  ...
  for (size_t i = 0; i < population_.size(); ++i) {
    ...
    double penalized_score =
        population_[i].get_score() /
        std::accumulate(similarity[i].begin(), similarity[i].end(), -1.0);
    population_[i].set_score(penalized_score);
  }
}

template <typename TargetType>
void GeneticAlgorithm<TargetType>::keep_elite(
  ...
  std::partial_sort(sorted_pop.begin(),
                    sorted_pop.begin() + params_.elite_count, sorted_pop.end(),
                    [](const Solution &a, const Solution &b) { return a > b; });
...
template <typename TargetType>
void GeneticAlgorithm<TargetType>::mutate(
    std::vector<Solution> &offspring) const {
...
template <typename TargetType>
void GeneticAlgorithm<TargetType>::crossover(
    std::vector<Solution> &offspring) const {
  std::uniform_real_distribution<double> prob_dist(0.0, 1.0);

  for (size_t i = params_.elite_count; i + 1 < offspring.size(); i += 2) {
    if (prob_dist(rng_) < params_.crossover_rate) {
      auto [c1, c2] = Solution::crossover_single_point(
          offspring[i], offspring[i + 1], data_.get_tree(), rng_);
...
template <typename TargetType>
void GeneticAlgorithm<TargetType>::replace_population(
...
template <typename TargetType> GAResults GeneticAlgorithm<TargetType>::run() {
  initialize();
...
template <typename TargetType>
Rcpp::NumericMatrix
GeneticAlgorithm<TargetType>::population_dissimilarity() const {
  auto [M, index] = dissimilarity_input_computation();
