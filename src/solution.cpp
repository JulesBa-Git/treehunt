...
size_t Solution::compute_hash_internal() const {
  ...
  for (int node : selected_nodes_) {
    // Boost hash_combine formula with 64 bits golden ratio
    seed ^= std::hash<int>{}(node) + 0x9e3779b97f4a7c15ULL + (seed << 6) + (seed >> 2);
  }

  return seed;
}

Solution::Solution(std::vector<int> nodes)
    : selected_nodes_{std::move(nodes)}, score_{0.0}, score_computed_{false},
...
Solution Solution::create_random_valid(const tree_structure &tree,
                                       std::mt19937 &rng, size_t target_size,
                                       int max_attempts, bool exact_size) {
  ...
  Rcpp::warning("Could not generate valid solution in max_attempts. Returning "
                "single node.");
...
Solution::determine_vertex(const tree_structure &tree) const {
...
Solution Solution::mutate_add_remove_type1(const tree_structure &tree,
  ...
  if (selected_nodes_.size() <= 1 || prob <= alpha / selected_nodes_.size()) {
    std::uniform_int_distribution<int> nodes_sampler(
        0, tree.get_depth().size() - 1);
    int new_node = -1;
    ...
    if (new_node == -1) {
      Rcpp::warning(
          "In mutation 1, couldn't add a candidate to the solution in " +
    ...
    std::uniform_int_distribution<size_t> index_sampler(
        0, selected_nodes_.size() - 1);
...
std::pair<Solution, Solution> Solution::crossover_single_point(
    ...
    const tree_structure &tree, std::mt19937 &rng) {
  const auto &depths = tree.get_depth();
  const auto &upper_bound = tree.get_upper_bound();
  ...
  std::uniform_int_distribution<int> internal_node_sampler(
      0, non_leaf_nodes.size() - 1);
  ...
  if (!returned_pair.first.is_valid(tree) ||
      !returned_pair.second.is_valid(tree))
    return {parent1, parent2};
